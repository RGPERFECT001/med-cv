...
default_scorer = strmatcher.WRatio
...
def extractWithoutOrder(query, choices, processor=default_processor, scorer=default_scorer, score_cutoff=0):
    ...
    if scorer in [strmatcher.WRatio, strmatcher.QRatio,
                  strmatcher.token_set_ratio, strmatcher.token_sort_ratio,
                  strmatcher.partial_token_set_ratio, strmatcher.partial_token_sort_ratio,
                  strmatcher.UWRatio, strmatcher.UQRatio] \
            and processor == utils.full_process:
        processor = no_process

    # Only process the query once instead of for every choice
    if scorer in [strmatcher.UWRatio, strmatcher.UQRatio]:
        pre_processor = partial(utils.full_process, force_ascii=False)
        scorer = partial(scorer, full_process=False)
    elif scorer in [strmatcher.WRatio, strmatcher.QRatio,
                    strmatcher.token_set_ratio, strmatcher.token_sort_ratio,
                    strmatcher.partial_token_set_ratio, strmatcher.partial_token_sort_ratio]:
...
def dedupe(contains_dupes, threshold=70, scorer=strmatcher.token_set_ratio):
